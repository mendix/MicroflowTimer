define([
  ...
  return declare("MicroflowTimer.widget.MicroflowTimer", [_WidgetBase], {
    ...
    postCreate: function () {
      ...
      if (
        !(
          (this.microflow && this.callEvent == "callMicroflow") ||
          (this.nanoflow.nanoflow && this.callEvent == "callNanoflow")
        )
      ) {
        ...
        if (this.firstIntervalAttr) {
          ...
          if (this.once) {
            this._timeout = setTimeout(
              lang.hitch(this, this._executeEvent),
              firstInterval
            );
          } else {
            if (this.startatonce) {
      ...
      if (this.microflow) {
        var mfObject = {
          ...
          error: lang.hitch(this, function (error) {
            logger.error(
              this.id + ": An error ocurred while executing microflow: ",
              error
            );
            this._flowRunning = false;
          }),
        ...
        if (
          !mx.version ||
          (mx.version && parseInt(mx.version.split(".")[0]) < 6)
        ) {
          mfObject.store = {
            caller: this.mxform,
          };
      ...
      if (this.nanoflow.nanoflow && this.mxcontext) {
        mx.data.callNanoflow({
          ...
          callback: lang.hitch(this, function (result) {
            if (!result) {
              logger.debug(
                this.id + "._executeNanoFlow callback, stopping timer"
              );
              this._stopTimer();
            }
            this._flowRunning = false;
          ...
          error: lang.hitch(this, function (error) {
            logger.error(
              this.id + ": An error ocurred while executing nanoflow: ",
              error
            );
            this._flowRunning = false;
          }),
